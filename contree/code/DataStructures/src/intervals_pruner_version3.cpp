// intervals_pruner.cpp  (thread-safe version — “do the same”)
...
IntervalsPruner::IntervalsPruner(const std::vector<int>& possible_split_indexes_ref, int max_gap)
    ...
    // IMPORTANT: resize (NOT reserve) so every index is valid immediately.
    // This prevents races / UB when threads read evaluated_indices_record[i]
    // before that i was ever written.
...
void IntervalsPruner::interval_shrinking(IntervalsPruner::Bound& current_bounds, int current_best_score) {
    ...
    if (current_bounds.last_split_left_index != -1) {
        ...
        const int updated_score_difference = record.first + record.second - current_best_score + max_gap;

        if (updated_score_difference > 0) {
            const int extended_left_bound =
                possible_split_indexes[current_bounds.last_split_left_index] + updated_score_difference + 1;

            if (extended_left_bound <= possible_split_indexes[current_bounds.right_bound]) {
                const int new_left_bound = int(
                    std::lower_bound(
                        possible_split_indexes.begin() + current_bounds.left_bound,
                        possible_split_indexes.begin() + current_bounds.right_bound,
                        extended_left_bound
                    ) - possible_split_indexes.begin()
                );
    ...
    if (current_bounds.last_split_right_index != -1) {
        ...
        if (updated_score_difference > 0) {
            const int extended_right_bound =
                possible_split_indexes[current_bounds.last_split_right_index] - updated_score_difference - 1;

            if (extended_right_bound >= possible_split_indexes[current_bounds.left_bound]) {
                const int new_right_bound = int(
                    std::upper_bound(
                        possible_split_indexes.begin() + current_bounds.left_bound,
                        possible_split_indexes.begin() + current_bounds.right_bound,
...
std::pair<int, int> IntervalsPruner::neighbourhood_pruning(int score_difference, int left, int right, int split_index) {
    ...
    const int minimum_right_value = possible_split_indexes[split_index] + score_difference + 1;

    if (minimum_right_value < possible_split_indexes[right]) {
        new_bound_left = int(
            std::lower_bound(
                possible_split_indexes.begin() + new_bound_left,
                possible_split_indexes.begin() + right,
                minimum_right_value
            ) - possible_split_indexes.begin()
        );
    } else {
    ...
    const int minimum_left_value = possible_split_indexes[split_index] - score_difference - 1;

    if (minimum_left_value > possible_split_indexes[left]) {
        new_bound_right = int(
            std::upper_bound(
                possible_split_indexes.begin() + left,
                possible_split_indexes.begin() + new_bound_right,
                minimum_left_value
            ) - possible_split_indexes.begin()
        );
    } else {
